...
class ServerRPArchiveExtra(commands.Cog, TC_Cog_Mixin):
    ...
    async def summarize_day(self, ctx, daystr: str, endstr: str = None):
        ...
        for sep in ChannelSep.get_all_separators_on_dates(
            ...
            await mt.editw(
                min_seconds=15,
                content=f"<a:LetWalk:1118184074239021209> Currently on Separator {ecount} ({sep.message_count}),message {mcount}.  Tokensize is {tokens}",
            )
            location = format_location_name(sep)
            if tokens > 16384:
                await ctx.send(
                    "I'm sorry, but there's too much content on this day for me to summarize."
                )
                return
            script += "\n" + location + "\n"
            ...
            for m in messages:
                ...
                if count > 5:
                    ...
                    await mt.editw(
                        min_seconds=15,
                        content=f"<a:LetWalk:1118184074239021209> Currently on Separator {ecount},message {mcount}.  Tokensize is {tokens}",
                    )
                    if tokens > 16384:
                        await ctx.send(
                            "I'm sorry, but there's too much content on this day for me to summarize."
        ...
        async with ctx.channel.typing():
            ...
            for pa in page.pages:
                embed = discord.Embed(title="summary", description=pa[:4028])
