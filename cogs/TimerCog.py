...
class TimerCog(commands.Cog, TC_Cog_Mixin):
            ...
            async with await DatabaseSingleton.get_async_session() as session:
                ...
                if expired:
                    for t in expired:
                        ...
                        if message:
                            await message.reply(
                                f"<@{t.user_id}>, Your {t.name} timer is done."
                            )
                        else:
                            try:
    ...
    @commands.command(
        name="start_alarm", description="Set a one time alarm.", extras={}
    )
    async def set_reminder(
        ...
        timer = await TimerTable.add_timer(
            ctx.author.id, name, alarm_time, message.jump_url
        )
    ...
    @LibParam(
        ...
        total_seconds="the total amount of seconds the timer will run for.",
    )
    @commands.command(
        name="start_timer", description="Get the current UTC Time", extras={}
    )
    async def start_timer(
        self, ctx: commands.Context, comment: str, name: str, total_seconds: int
    ):
        # This is an example of a decorated discord.py command.
        bot = ctx.bot
        now = datetime.now()
        ...
        timer = await TimerTable.add_timer(
            ctx.author.id, name, target, message.jump_url
        )
    ...
    @commands.command(
        name="event_schedule", description="schedule an event.", extras={}
    )  # Command decorator.
    @commands.guild_only()
    async def schedule_event(
        ...
        channel: discord.VoiceChannel = discord.utils.get(
            ctx.guild.channels, name=channelname.replace("#", "")
        )
        # Permission check
        if not ctx.author.guild_permissions.manage_events:
            return await ctx.send(
                "You do not have permission to create scheduled events."
            )
        if not ctx.guild.me.guild_permissions.manage_events:
            return await ctx.send(
                "I do not have permission to create scheduled events."
            )
