...
class AIMessageTemplates(MessageTemplates):
    @staticmethod
    def get_invoke_audit_embed(message:Message, chat: Optional[ChatCreation]=None, color:int=0xffffff):
       ...
       #add a message to the 'audit channel'
        ...
        return emb
    @staticmethod
    def get_response_audit_embed(message:Message, chat: Optional[Any]=None, color:int=0xffffff):
        '''Create an embed that sums up the server archive information for this server.'''
       #add a message to the 'audit channel'
        emb=Embed(title="Audit",description=f"```{message.content}```")
        if chat:
    ...
    async def add_resp_audit(
        ...
        **kwargs):
        ...
        if audit_channel:
            ...
            embed.add_field(name="Server Data",value=f"{guild.name}, \nServer ID: {guild.id}",inline=False)
            embed.add_field(name="User Data",value=f"{user.name}, \n User ID: {user.id}",inline=False)
    ...
    async def add_user_audit(
        ctx:commands.Context, 
        chat: Optional[ChatCreation]=None, 
        **kwargs):
        '''Create an embed'''
    ...
    (
        ...
        audit_channel=AssetLookup.get_asset("monitor_channel")
        if audit_channel:
            embed=AIMessageTemplates.get_function_audit_embed(functiondict,ctx.message,name,args)
