...
class StarboardCog(commands.Cog):
                ...
                if mess:
                    ...
                    if entry and starboard and starboard_channel:
                        if entry.total > starboard.threshold:
                            ...
                            entry = await StarboardEntryTable.add_or_update_bot_message(message.guild.id, message.id, bm.id,bm.jump_url)
                        else:
                            await StarboardEntryTable.delete_entry_by_bot_message_url(bot_message)
                    else:
                        await StarboardEntryTable.delete_entry_by_bot_message_url(bot_message)
    ...
    async def add(self, ctx, channel: discord.TextChannel, threshold: int):
        ...
        await ctx.send(f"Starboard added to {channel.mention} with a threshold of {threshold} stars.")
    ...
    async def reaction_action(self, fmt: str, payload: discord.RawReactionActionEvent) -> None:
        try:
            ...
            if starboard:
                ...
                return
            if (discord.utils.utcnow()-discord.utils.snowflake_time(payload.message_id))>datetime.timedelta(days=7):
                return
            async with self.lock:
                ...
                if fmt=='star':
                    ...
                    await StarboardEntryTable.add_or_update_entry(guild.id, message.id, message.channel.id, message.author.id,message_url=url)
                else:
                    entry=await StarboardEntryTable.get_entry(guild.id,message.id)
                    if not entry:
                        return
                    await StarboardEntryTable.add_or_update_entry(guild.id, message.id, message.channel.id, message.author.id,url,-1)
                entry=await StarboardEntryTable.get_entry(guild.id,message.id)
                await self.update_starboard_message(guild, message, entry.bot_message_url)
    ...
    async def update_starboard_message(self, guild, message, bot_message):
        ...
        if not bot_message:
            content, embed= self.get_emoji_message(message,entry.total)
            bm=await starboard_channel.send(content, embed=embed)
            entry = await StarboardEntryTable.add_or_update_bot_message(guild.id, message.id, bm.id,bm.jump_url)
        else:
