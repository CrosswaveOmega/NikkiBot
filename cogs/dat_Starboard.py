...
class Starboard(Base):
    ...
    async def add_starboard(cls, guild_id: int, channel_id: int, threshold: int):
        async with DatabaseSingleton.get_async_session() as session:
            starboard = cls(id=guild_id, channel_id=channel_id, threshold=threshold, locked=False)
...
class StarboardEntryTable(Base):
    ...
    async def get_entry(cls, guild_id: int, message_id: int):
        async with DatabaseSingleton.get_async_session() as session:
            query = select(cls).where(cls.message_id == message_id, cls.guild_id == guild_id)
            result = await session.execute(query)
            return result.scalar()

    @classmethod
    async def add_or_update_entry(cls, guild_id: int, message_id: int, channel_id: int, author_id: int, message_url:str, op:int=1):
        async with DatabaseSingleton.get_async_session() as session:
            query = select(cls).where(cls.message_id == message_id, cls.guild_id == guild_id)
            result = await session.execute(query)
            entry = result.scalar()
            if entry:
                ...
                entry = cls(message_id=message_id, channel_id=channel_id, guild_id=guild_id, author_id=author_id, message_url=message_url,total=1)
                session.add(entry)
            await session.commit()
            return entry

    @classmethod
    async def add_or_update_bot_message(cls, guild_id: int, message_id: int, bot_message_id: int, bot_message_url:str):
        async with DatabaseSingleton.get_async_session() as session:
            query = select(cls).where(cls.message_id == message_id, cls.guild_id == guild_id)
            result = await session.execute(query)
            entry = result.scalar()
            if entry:
                ...
                entry = cls(message_id=message_id, guild_id=guild_id, bot_message=bot_message_id)
                session.add(entry)
            await session.commit()
            return entry
    ...
    async def delete_entry_by_bot_message_url(cls, bot_message_url: str):
        async with DatabaseSingleton.get_async_session() as session:
            result = await session.execute(select(cls).where(cls.bot_message_url == bot_message_url))
            entry = result.scalar()
            if entry:
