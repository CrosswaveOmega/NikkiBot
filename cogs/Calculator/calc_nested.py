...
def redo_current_expression_with_evaluated(current_expression, opDone, spartA="", spartB="", outputFunc=defaultOutput):
...
def dice_roll_op(current_expression,spartA, spartB, outputFunc):
    ...
    for i in current_expression:
        if i.value == "d": #operator for dice
            ...
            if ((position-1) <0): #the position is less than zero, so don't check there!
                valid_amount_check=False
            elif (current_expression[position-1].type!=DatabitType.NUMBER):
                valid_amount_check=False

            if ((position+1) >=len(current_expression)):#The position is too big!  Don't check there
            ...
            if 0 <= position+4 < len(current_expression):
                ...
                if(comp3==">"):
                    ...
                    full_string=" if a amount greater than {} is rolled".format(checkValue)
                    outputStrPart2+=choosestring(full_string,"if roll>{}".format(checkValue))
                    moreMode=True
                ...
                if(comp3=="<"):
                    full_string=" if a amount less than {} is rolled".format(checkValue)
                    outputStrPart2+=choosestring(full_string,"if roll<{}".format(checkValue))
                    lessMode=True
                    current_expression[position+3].enabled=False
                    current_expression[position+4].enabled=False
                ...
                if(comp3=="≥"):
                    moreMode=True
                    equals=True
                    full_string=" if a amount greater than or equal to {} is rolled".format(checkValue)
                    outputStrPart2+=choosestring(full_string,"if roll≥{}".format(checkValue))
                    current_expression[position+3].enabled=False
                    current_expression[position+4].enabled=False
                    checkValue=comp4
                if(comp3=="≤"):
                    lessMode=True
                    equals=True
                    full_string=" if a amount less than or equal to {} is rolled".format(checkValue)
                    outputStrPart2+=choosestring(full_string,"if roll≤{}".format(checkValue))
                    current_expression[position+3].enabled=False
                    current_expression[position+4].enabled=False
            ...
            if int(dice_sides)<=0:
                outputFunc.outFunc("...You can't roll a {} sided dice!".format(str(dice_sides)))
                raise Exception
            outputStr=":game_die:Roll {}d{} {}:".format(str(dice_amount), str(dice_sides), outputStrPart2Format)
            ...
            for x in range(dice_amount):
                ...
                if(explodeMode): #EXPLODING dice
                    boolean=compareValue(rollvalue, rerollValue, lessMode, moreMode, equals)
                    ...
                    while(boolean and terminationCount<=100000):
                        ...
                        if compoundMode:
                            ...
                            displayvalue+=short
                            if(printMode):
                                outputFunc.outFunc("Compounding, adding ",rollvalue, ". new value is",sumV)
                        else:
                ...
                if(terminationCount>=99999):
                    outputFunc.outFunc("failure limit reached.  Terminating for sake of sanity.")
                    raise Exception
    ...
    current_expression=redo_current_expression_with_evaluated(current_expression, opDone, spartA, spartB, outputFunc)
    if didrun:
        if outputFunc.mode=="short":
            printOp(current_expression,spartA,spartB,outputFunc,verbop=1)
    return current_expression,  didrun

def drop_and_keep(current_expression:List[DataBit], spartA:str, spartB:str, outputFunc):
    opDone=False
    nextroll=0
    rall=[1]
    position=0
    ...
    for i in current_expression:
        if i.value == "δ":
            ...
            outputStr="Dropping {} low values from {}.".format( str(dropNum),str(rolling))
        ...
        if i.value == "κ":
            ...
            outputStr="Keeping {} high values from {}.".format(str(dropNum),str(rolling))
            outputFunc.outFunc(outputStr, verb=1,isop=True)
...
def parse_and_calculate_string(strings:str, spartA:str, spartB:str, outputFunc=defaultOutput):
    ...
    current_expression, didNewThing = dice_roll_op(current_expression, spartA, spartB, outputFunc)

    #DropAndKeep
    #δ
    #κ
    current_expression,didNewThing= drop_and_keep(current_expression, spartA, spartB, outputFunc)

    #Sum Values in Sequence
    #Σ[1,2,...,n]
    current_expression, didNewThing = sequence_sum(current_expression, spartA, spartB, outputFunc)
    preformed_printworthy_op=preformed_printworthy_op or didNewThing
    #Average of values in sequence
    #℘[1,2,...,n]
    current_expression, didNewThing = sequence_avg(current_expression, spartA, spartB, outputFunc)
    preformed_printworthy_op=preformed_printworthy_op or didNewThing
    #Exponentiation operation
    current_expression, didNewThing = exponent_math(current_expression, spartA, spartB, outputFunc)
    preformed_printworthy_op=preformed_printworthy_op or didNewThing
    outputFunc.outFunc("DEBUG STATEMENT 3",verb=7)

    #Factorial Operation
    current_expression, didNewThing = factorial_math(current_expression, spartA, spartB, outputFunc)
    preformed_printworthy_op=preformed_printworthy_op or didNewThing
    
    #Multiplication
    current_expression, didNewThing = multiplication_math(current_expression, spartA, spartB, outputFunc)
    preformed_printworthy_op=preformed_printworthy_op or didNewThing
    #New Division
    current_expression, didNewThing = division_math(current_expression, spartA, spartB, outputFunc)
    preformed_printworthy_op=preformed_printworthy_op or didNewThing
    #DirectSum, o(n)
    current_expression, didNewThing = DirectSum_math(current_expression, spartA, spartB, outputFunc)
    preformed_printworthy_op=preformed_printworthy_op or didNewThing

    #Addition- Will Repeat if it does a op.
    #O(n)
    current_expression, didNewThing = addition_math(current_expression, spartA, spartB, outputFunc)
    preformed_printworthy_op=preformed_printworthy_op or didNewThing

    #Subtraction.  
    current_expression, didNewThing = subtraction_math(current_expression, spartA, spartB, outputFunc)
    preformed_printworthy_op=preformed_printworthy_op or didNewThing
