...
class GameMenu(discord.ui.View):
    ...
    def reload(self):
        ...
        for b, l in menu.items():
            button = MenuButton(
                label=b, style=discord.ButtonStyle.primary, custom_id=b, row=1
            )
            self.add_item(button)
        button = MenuButton(
            label="back", style=discord.ButtonStyle.primary, custom_id="back", row=1
        )
        self.add_item(button)
        return "done"
    ...
    async def confirm(
        ...
        if button.custom_id == "back":
            ...
            self.chain.append(button.custom_id)
        gui.gprint(self.chain)
        ...
        if reload_status == "end":
            ...
            await interaction.response.edit_message(content=button.custom_id, view=self)
