...
class MusicCog(commands.Cog, TC_Cog_Mixin):
    ...
    async def music(self, interaction: discord.Interaction) -> None:
        ...
        await interaction.response.send_message(
            "Hello from top level command!", ephemeral=True
        )
    ...
    async def connect(self, interaction: discord.Interaction):
        ...
        if isinstance(voice, type(None)) or not voice.is_connected():
            ...
            await MessageTemplatesMusic.music_msg(
                ctx, "Connected", "I'm connected to **Voice Channel!**"
            )
            ...
            await MessageTemplatesMusic.music_msg(
                ...
                "I'm connected to another **Voice Channel** already!",
            )
        else:
            vc_name = voice.channel.name
            await MessageTemplatesMusic.music_msg(
                ctx, "Already connected", f"I'm already connected to {vc_name}"
            )
    ...
    async def disconnect(self, interaction: discord.Interaction):
        ...
        if isinstance(voice.is_connected(), type(None)):
            await MessageTemplatesMusic.music_msg(
                interaction, "Not connected", f"I'm not connected to {vc_name}"
            )
        elif interaction.user.voice.channel != ctx.voice_client.channel:
            await MessageTemplatesMusic.music_msg(
                ctx,
                "Connected to another",
                "I'm connected to another voice channel at the moment...",
            )
        else:
            await MessageTemplatesMusic.music_msg(
                ctx, "Disconnected", f"I am now disconnected from {vc_name}, bye-bye!"
            )
    ...
    @mp.command(
        name="reconnect",
        description="Nikki disconnects and reconnects from Voice Channel",
    )
    ...
    @mp.command(
        name="play",
        description="Play song to Voice Channel.  If you have a valid url, you can add it here.",
    )
    @app_commands.describe(
        url=" URL to audio (YouTube,Soundcloud).  If it is not a url, then Nikki will use it as a search term."
    )
    async def play(self, interaction: discord.Interaction, url: str = ""):
        ...
        if isinstance(voice, type(None)) or not voice.is_connected():
            ...
            await MessageTemplatesMusic.music_msg(
                ctx,
                "Connected",
                f"I'm now connected to {interaction.user.voice.channel.name}!",
            )

        elif await connection_check(interaction, ctx, 3):
            await MessageTemplatesMusic.music_msg(
                ctx, "Connected to another", "I'm connected to another Voice Channel."
            )
            return

        if (
            MusicManager.get(guild).voice == None
            or MusicManager.get(guild).channel == None
        ):
            await MusicManager.get(guild).setvoiceandctx(interaction)
        if url:
            attempt = await ctx.send(
                "<a:trianglepointer:1132773635195686924> attempting to add"
            )
            song: AudioContainer = await MusicManager.get(guild).playlist_actions(
                "add_url", (url, ctx.author)
            )
            ...
            if MusicManager.get(guild).player_condition != "play":
                ...
                await MusicManager.get(guild).send_message(
                    ctx,
                    "Play",
                    f"There's already something playing, \
                    so I'll just add {song.link_markdown()} to my playlist at spot{length}.",
                )
        else:
            attempt = await ctx.send(
                "<a:trianglepointer:1132773635195686924> starting player."
            )
    ...
    async def pause(self, interaction: discord.Interaction):
        ...
        if await connection_check(
            interaction, ctx
        ):  # if it's true, then it shouldn't run.
            return
        voice = discord.utils.get(self.bot.voice_clients, guild=interaction.guild)
    ...
    async def send_player(self, interaction: discord.Interaction):
        ...
        newm = await ctx.send(
            "Player message, will be rendered useless after 3 minutes.",
    ...
    @mp.command(
        name="nowplaying",
        description="View the song currently playing, and send player buttons.",
    )
    async def now_playing(self, interaction: discord.Interaction):
        ...
        if not MusicManager.get(guild).current:
            await MusicManager.get(guild).send_message(
                ctx, "No music", "I'm not playing any music right now."
            )
            # await self.send_player(interaction)
            return
        data = MusicManager.get(guild).current
        await MusicManager.get(guild).send_message(
            ctx, "Now Playing", f"I am currently playing {data.title}!"
        )
        # await self.send_player(interaction)
    ...
    async def resume(self, interaction: discord.Interaction):
        ...
        await MusicManager.get(guild).player_actions(
            "play", ctx
        )  # Just a stand in for play.

    @mp.command(name="repeat", description="Enable/Disable current song repeating")
    @app_commands.describe(action="Repeat type.")
    async def repeat(
        self, interaction: discord.Interaction, action: Literal["none", "all", "one"]
    ):
        ctx: commands.Context = await self.bot.get_context(interaction)
        guild: discord.Guild = interaction.guild
        ...
        if action == "all":
            ...
            MusicManager.get(guild).repeatone = False
            await MusicManager.get(guild).send_message(
                ctx, "Repeat", "I've turned **repeat** mode on."
            )
        if action == "one":
            MusicManager.get(guild).repeat = False
    ...
    def make_playlist_embeds(self, interaction):
        ...
        if MusicManager.get(guild).songs:
            duration = 0.0
            processing = MusicManager.get(
                guild
            ).processsize  # Check size of songs still getting the data from.
            pstr = ""
            if processing > 0:
                pstr = f"{processing}"
            for song in MusicManager.get(guild).songs:
            ...
            def transform(current):
                off, titles = current
                playlist = "".join(
                    f"**{idv}:** [{song.title}]({song.url})-{song.duration}\n"
                    for idv, song in titles
                )
                emb = MusicManager.get(guild).get_music_embed(
                    "pl", f"**Playlist**\n{playlist}"
                )
                if pstr:
                    emb.add_field(
                        name="Still Processing",
                        value=f"I'm still processing {pstr} songs!",
                    )
            ...
            embeds = speciallistsplitter(
                MusicManager.get(guild).songs,
                resetdata,
                splitcond,
                transform,
                addtransform,
    ...
    @mp.command(
        name="playlist",
        description="Show the songs you currently have in your Playlist",
    )
    ...
    async def playlistadd(self, interaction: discord.Interaction, url: str):
        ...
        with yt_dlp.YoutubeDL(ydlops) as ydl:
            try:
                ...
                gui.gprint(e)
            opres = await MusicManager.get(guild).playlist_actions(
                "add_url", (url, ctx.author)
            )
            if opres == None:
                await MessageTemplatesMusic.music_msg(
                    ctx,
                    "something went wrong...",
                    f"I couldn't find the **{url}** video... are you sure it's a valid url?",
                )
                return
            await MessageTemplatesMusic.music_msg(
                ctx, "Playlist", f"**I added {opres.title}** to my playlist."
            )

    @mp.command(
        name="add_server_playlist",
        description="get all music tracks saved inside one channel.",
    )
    @app_commands.describe(
        channel="The Discord channel in this server the videos are in."
    )
    @app_commands.describe(
        messagecount="Total number of messages in channel to check.  Default is 1000."
    )
    async def playlistaddchannel(
        self,
        ...
        with yt_dlp.YoutubeDL(ydlops) as ydl:
            ...
            async for thisMessage in channel.history(limit=messagecount):
                ...
                adjusted_average = adjusted_average + (
                    (value - adjusted_average) / min(counter, 200)
                )
                ...
                for i, item in enumerate(urls):
                    ...
                    total += 1
        stat.delete()
        await MusicManager.get(ctx.guild).send_message(
            ctx,
            "Playlist",
            f"I added all {total} tracks to my processing queue! \
    ...
    async def radioadd(
        ...
        if filesize >= Bytelimit:
            await MessageTemplatesMusic.music_msg(
                ctx,
                "filecheck",
                f"This file is {filesize} megabytes large, my upper limit is 25mb!",
            )
            return
        ...
        await MessageTemplatesMusic.music_msg(
            ctx, "filecheck", f"Uploaded {file.filename} to my radio folder!"
        )
        await ctx.bot.get_channel(ctx.bot.error_channel).send(
            f"Uploaded {file.filename} into the radio folder."
        )

    @mp.command(
        name="playlistcopy", description="get multiple songs from youtube playlist"
    )
            ...
            if "entries" in res:
                ...
                if l >= 256:
                    await MessageTemplatesMusic.music_msg(
                        ctx,
                        "something went wrong...",
                        f"...I am not adding {l} songs from this playlist...",
                    )
                    return
    ...
    async def playlistjump(self, interaction: discord.Interaction, spot: int):
        ...
        await MessageTemplatesMusic.music_msg(
            ctx,
            "Playlist",
            f"**I moved {result.title}** to the front of my playlist.",
            result.to_display_dict(),
        )
    ...
    async def playlistremove(self, interaction: discord.Interaction, spot: int):
        ...
        await MessageTemplatesMusic.music_msg(
            ctx,
            "Playlist",
            f"**I removed {result.title}** from my playlist.",
            result.to_display_dict(),
        )
    ...
    async def playlistclear(self, interaction: discord.Interaction):
        ...
        if res == "done":
            await MessageTemplatesMusic.music_msg(
                ctx, "Clear Playlist", "I cleared out my playlist."
            )
        else:
            await MessageTemplatesMusic.music_msg(
                ctx, "Empty Playlist", "My playlist is already empty."
            )
    ...
    async def playlistshuffle(self, interaction: discord.Interaction):
        ...
        if res == "done":
            await MessageTemplatesMusic.music_msg(
                ctx, "Shuffle", "I've shuffled my playlist!"
            )
        else:
            await MessageTemplatesMusic.music_msg(ctx, "?", f"{res}")
