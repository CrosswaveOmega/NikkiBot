...
class ArchiveCompiler:
    ...
    async def group(self,m,profile:ServerArchiveProfile):
        ...
        ts, group_id = await do_group(self.guild.id, profile.last_group_num, ctx=self.ctx)
        ...
        total_time_for_cluster = ((fullcount-self.m_arc) * avgtime) +((group_id) * avgsep)
    ...
    async def post_setup(self, m: discord.Message, profile: ServerArchiveProfile, archive_channel: discord.TextChannel,upper_lim:Optional[int]=None) ->  Tuple[StatusEditMessage,List[ChannelSep]]:
        """
    ...
    async def post_groups(self, mt, grouped:List[ChannelSep], profile:ServerArchiveProfile,archive_channel:discord.TextChannel):
    ...
    #done
    async def post(self, m, profile:ServerArchiveProfile,archive_channel:discord.TextChannel,upper_lim:Optional[int]=None):
        mt,grouped=await self.post_setup(m,profile,archive_channel,upper_lim=upper_lim)
        didthing=False
        if len(grouped)>0:
