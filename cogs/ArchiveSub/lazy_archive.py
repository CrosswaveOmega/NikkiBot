...
async def lazy_archive(self, ctx):
    ...
    if lazycontext.state == "setup":
        ...
        me = await ctx.channel.send(
            content=f"<a:LetWalk:1118184074239021209> currently on {lazycontext.archived_so_far}/{lazycontext.message_count}, will take {seconds_to_time_string(upper_time_limit())} this session."
        )
        ...
        while upper_time_limit() > 0:
            ...
            grouped = ChannelSep.get_unposted_separators(
                guildid, limit=CHANNEL_SEPS_PER_CLUSTER
            )
            ...
            if len(grouped) <= 0:
                ...
                session = DatabaseSingleton.get_session()
                query = (
                    session.query(func.date(ArchivedRPMessage.created_at), func.count())
                    .filter(
                        (ArchivedRPMessage.server_id == guildid)
                ...
                ts, group_id = await do_group(
                    guildid, profile.last_group_num, ctx=ctx, upperlim=thelim
                )
            ...
            length = len(grouped)
            for sep in grouped:
                message_total += sep.get_message_count()
            ...
            for e, sep in enumerate(grouped):
                ...
                for amess in sep.get_messages():
                    ...
                    await mt.editw(
                        min_seconds=45,
                        content=f"<a:LetWalk:1118184074239021209> currently on {lazycontext.archived_so_far}/{lazycontext.message_count}, remaining session time is at least {seconds_to_time_string(upper_time_limit(total_time_for_cluster))}",
                    )
