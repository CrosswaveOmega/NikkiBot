...
class PollTable(PollingBase):
    ...
    def vote(button_id, user_id):
        ...
        poll_data = (
            session.query(PollData)
            .filter(and_(PollData.poll_id == poll_id, PollData.user_id == user_id))
            .first()
        )
        ...
        if poll_data:
            ...
            poll.change_vote = True
            poll_data = PollData(
                poll_id=poll_id, user_id=user_id, choice=choice.split("_")[1].upper()
            )
    ...
    def update_poll_status():
        ...
        current_time = datetime.now()
        polls_to_update = (
            session.query(PollTable)
            .filter(PollTable.end_date <= current_time, PollTable.active == True)
            .all()
        )
    ...
    def get_active_polls(server_id):
        ...
        for poll in session.query(PollTable).filter(
            ...
            (
                ...
                | ((PollTable.scope == "server") & (PollTable.server_id == server_id))
            ),
        ):
    ...
    def poll_embed_view(poll):
        ...
        active = poll.active
        embs = [
            "<:resp_A:1107355323758546977>",
            "<:resp_B:1107355324979093545>",
            "<:resp_C:1107355327302746152>",
            "<:resp_D:1107355331836788798>",
            "<:resp_E:1107355337171927090>",
        ]
        ...
        embed = discord.Embed(
            title=f"<:voteicon:1106567262086901821>{poll_name}",
        ...
        for i, (button_id, choice_text) in enumerate(poll.poll_buttons()):
            t = tally[i + 1]
            embed.add_field(
                name=f"{embs[i]}:{choice_text}", value=f"{t} Votes", inline=True
            )
    ...
    def poll_list(poll_ids):
        ...
        for i in range(0, len(polls), 5):
            ...
            for poll in polls[i : i + 5]:
                name = (
                    f"{poll.poll_name}, id:(`{poll.poll_id}`), hex:(`{poll.poll_hex}`)"
                )
...
class PollMessages(PollingBase):
    ...
    def get_active_poll_messages() -> List[Tuple[PollTable, List[Tuple[int, str]]]]:
        ...
        poll_tables = (
            session.query(PollTable)
            .filter(PollTable.active == True)
            .options(joinedload(PollTable.poll_messages))
            .all()
        )
        result = []
        for poll_table in poll_tables:
            messages = [
                (poll_message.message_id, poll_message.message_url)
                for poll_message in poll_table.poll_messages
            ]
    ...
    def remove_invalid_poll_messages() -> int:
        session = DatabaseSingleton.get_session()
        invalid_poll_messages = (
            session.query(PollMessages)
            .join(PollTable)
            .filter(PollTable.active == False)
            .all()
        )
    ...
    def remove_poll_message(message_id: int) -> int:
        session = DatabaseSingleton.get_session()
        poll_message = (
            session.query(PollMessages).filter_by(message_id=message_id).first()
        )
        if poll_message:
            session.delete(poll_message)
            session.commit()
...
class PollChannelSubscribe(PollingBase):
    ...
    def set_or_update(channel_id, server_id):
        session = DatabaseSingleton.get_session()
        entry = (
            session.query(PollChannelSubscribe).filter_by(server_id=server_id).first()
        )
        if entry:
