...
class Poll_Choice_Make(discord.ui.Modal, title='Edit your poll choices.'):
    ...
    def __init__(self, scope: str, choices: int, *args, **kwargs):
        ...
        self.choice_a = discord.ui.TextInput(label='Choice A', max_length=32,required=True)
        self.choice_b = discord.ui.TextInput(label='Choice B',max_length=32,required=True)
        self.add_item(self.choice_a)
        self.add_item(self.choice_b)
        if self.choices >= 3:
            self.choice_c = discord.ui.TextInput(label='Choice C',max_length=32,required=True)
            self.add_item(self.choice_c)
        if self.choices >= 4:
            self.choice_d = discord.ui.TextInput(label='Choice D',max_length=32,required=True)
            self.add_item(self.choice_d)
        if self.choices == 5:  
            self.choice_e = discord.ui.TextInput(label='Choice E',max_length=32,required=True)
            self.add_item(self.choice_e)

    async def on_submit(self, interaction):
        choices = [(c,getattr(self, f"choice_{c}").value) for c in 'abcde'[:self.choices]]
        self.done={}
...
def format_poll_embed(poll_dict, op_status=''):
    ...
    if days > 0 or hours > 0 or minutes > 0:
        ...
        embed.add_field(name="Poll Duration:",value=f"Poll duration: {duration_str}", inline=False)
    if op_status:
        embed.add_field(name="[STATUS]",value=op_status)
    embed.set_footer(text=f"Scope: {poll_dict['scope']},  Server_ID: {poll_dict['server_id']}")
    return embed
...
class PollEdit(discord.ui.View):
    ...
    async def nameedit(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        if name_modal.done!=None:
            ...
            await interaction.edit_original_response(content='cancelled',embed=format_poll_embed(self.my_poll, "Modal change cancelled."))
        
    @discord.ui.button(label='Edit Poll Choices', style=discord.ButtonStyle.blurple)
    async def choiceedit(self, interaction: discord.Interaction, button: discord.ui.Button):
        '''
        ...
        if choice_modal.done!=None:
            ...
            await interaction.edit_original_response(content='cancel',embed=format_poll_embed(self.my_poll,"Choices modal closed out."))
    def complete_check(self):
        ...
        total_time =  self.my_poll['days'] * 24 * 60 +  self.my_poll['hours'] * 60 +  self.my_poll['minutes']
        if total_time <= 0:
    ...
    async def confirm(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        if cc!="OK":
            await interaction.response.edit_message(content=cc,embed=format_poll_embed(self.my_poll,cc))
        else:
            await interaction.response.edit_message(content="Making your poll...",embed=format_poll_embed(self.my_poll,'Completed!  Making your poll.'))
            self.value = True
            self.stop()
    ...
    async def cancel(self, interaction: discord.Interaction, button: discord.ui.Button):
        await interaction.response.edit_message(content='Canceled',embed=format_poll_embed(self.my_poll,'Operation cancelled.'))
        self.value = False
        self.stop()
