...
def render_planet(texture, xpix, ypix, sphere_center, sphere_radius, angle, light_dir):
    ...
    for y in range(21):
        for x in range(21):
            ...
            if dx**2 + dy**2 <= sphere_radius**2:  # Check if the point is within the circle
                ...
                u = 0.5 + (math.atan2(nz_rot, nx_rot) / (2 * math.pi))  # Calculate texture coordinate u
                ...
                light_intensity = np.dot(normal, light_dir)  # Calculate the light intensity
                light_intensity = max(0.0, light_intensity)  # Ensure a minimum light intensity
                ...
                color = tuple(int(c * light_intensity) for c in color)  # Adjust the color by the light intensity
                if color != (0, 0, 0):  # If the color is not black, set the pixel in the image
                    sphere_img.putpixel((x, y), color)  # Set the pixel color in the image

    return sphere_img

def generate_planet_texture(colors, num_craters, num_clouds, nme='', make_new_texture=True):
    ...
    sphere_center = (10, 10)
    sphere_radius = 10
    def create_gif_with_light_variation(texture, sphere_center, sphere_radius, frames, output_path):
        images = []
        for frame in range(frames):
            ...
            sphere_img=render_planet(texture,xpix,ypix,sphere_center,sphere_radius,angle,light_dir)
    ...
    create_gif_with_light_variation(texture, sphere_center, sphere_radius, frames, output_path)


def threshold_color(color_tuple):
    # Divide each RGB value by 32 and round up to the nearest integer
    print("before:",color_tuple)
    print("midstep",tuple(int(math.ceil(value / 8) ) for value in color_tuple) )
    thresholded_tuple = tuple(int(math.ceil(value / 8) * 8) for value in color_tuple)
    print("after",thresholded_tuple)    
...
print(all_colors)
all_colors = {key: np.array([threshold_color(color) for color in value]) for key, value in all_colors.items()}
print('as',all_colors)
print("")
