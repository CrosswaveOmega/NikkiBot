            ...
            if type == 'data':
                ...
                for source, subdata in sources.items():
                    if target['data']['source'] == source:
                        desoupeddata[source]=(target['data']['label'],remove_link_node(target['data']['value']))
    return desoupeddata
...
def extract_attack_table(html_content):
    ...
    for table in tables:
        ...
        for r in rows[1:]:
            ...
            print(name,value)
            if "Limit" in value: value.remove("Limit")
            if name=='Level Continued...':  name='Level'
...
def callsoup(params):
    # Send API request
    response = requests.get("https://toontown-corporate-clash.fandom.com/api.php", params=params)
    data = response.json()
    return data
def get_cog_soup(page):
    ...
    params = {
        "action": "parse", "page": page, "format": "json",'section': '0'
    }
    data=callsoup(params)
...
async def formatembed(url:str,soup:dict, desoup:dict, attack_soup:str,cheat_soup:str, foetattle:str='', cheat_tattle:str=''):
            ...
            for key, values in param.items():
                ...
                if key:
                    ...
                    if len(values)>2:
                        ...
                        else: values_str = f"{values[0]}-{values[-1]}"
    ...
    if cogclass in ['mook','mini','boss']:
        ...
        embed.add_field(name="Likes/Dislikes", value=f"{tform(desoup['likes'])}\n{tform(desoup['dislikes'])}", inline=True)
        attacks=""
        ...
        if cheat_tattle:
            print(cheat_tattle)
            embed.add_field(name=f"Cheat Tattle", value=cheat_tattle[:1020], inline=False)
