...
"""
            ...
            async for message in post.history(limit=300):
                if message.author.id == ctx.bot.user.id:
                    ...
                    if match:
                        ...
                        break
                        print(
                            f"Message ID: {message.id} contains timestamp: {dt_object}"
                        )
            if dt_object:
    ...
    async def role_mod(
        ...
        if role_obj:
            ...
            await ctx.send(
                f"Added modification for role {role_obj.name}", ephemeral=True
            )
            pass
        else:
    ...
    async def showleaderboard(self, ctx: commands.Context):
        ...
        for n, e in enumerate(leaderboard):
            ...
            outs.append(outv)
        embed = discord.Embed(
            title="Quest Leaderboard", description="\n".join(outs)[:4000]
        )
    ...
    async def make_expiry(self, ctx: commands.Context, duration: str):
        ...
        if not dtformat:
            await ctx.send("Quests are not supposed to last forever!")
            return
