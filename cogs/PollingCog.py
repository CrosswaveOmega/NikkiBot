...
class PersistentView(discord.ui.View):
    ...
    async def callback(self, interaction, button):
        ...
        await interaction.response.send_message(f'You are {user.name}, this is {label}, and you have pressed this button {self.my_count[str(user.id)]} times.', ephemeral=True)
    ...
    @discord.ui.button(label='Blue', style=discord.ButtonStyle.blurple, custom_id='persistent_view:blue')
...
class PollingCog(commands.Cog, TC_Cog_Mixin):
    ...
    async def make_poll(self, interaction: discord.Interaction, scope:Literal['server','global']='server'):
        ...
        if poll_edit_view.value==True:
            ...
            time_delta = timedelta(days=newpoll['days'], hours=newpoll['hours'], minutes=newpoll['minutes'])
