...
def make_playlist_embeds(player, interaction):
    ...
    if player.songs:
        ...
        processing = (
            player.processsize
        )  # Check size of songs still getting the data from.
        pstr = ""
        if processing > 0:
            pstr = f"{processing}"
        ...
        def transform(current):
            ...
            if pstr:
                emb.add_field(
                    name="Still Processing", value=f"I'm still processing {pstr} songs!"
                )
        ...
        plist = speciallistsplitter(
            player.songs, resetdata, splitcond, transform, addtransform
        )
        return plist
...
class MusicPlayer(PlaylistMixin, PlayerMixin):
    ...
    def __init__(self, bot, guild: discord.Guild = None):
        ...
        self.FFMPEG_FILEOPTIONS = {"options": "-vn -loglevel 0"}
    ...
    async def playlist_view(self, interaction: discord.Interaction):
        ...
        pagecall = PlaylistPageContainer(
            interaction, self
        )  # Page container for playlist
        return pagecall
    ...
    async def playlistcallback(
        self, interaction: discord.Interaction, action: str, param: Optional[Any] = None
    ):
        ...
        if self.song_add_queue.empty() == False:
            ...
            if self.songcanadd:
                ...
                try:
                    ...
                    got = True
                    await asyncio.gather(asyncio.to_thread(self.musicplayeradd, song))
                    ...
                    if self.song_add_queue.empty():
                        ...
                        if self.channel:
                            embed = discord.Embed(
                                description="All songs have been processed.",
                ...
                if len(voice.channel.members) <= 1:
                    ...
                    if self.timeidle >= 20:
                        if self.channel != None:
                            await self.send_message_internal(
                                self.channel,
                                "disconnected",
    ...
    async def send_message(
        ...
        try:
            await self.send_message_internal(
                ctx_to_try, title, desc, editinter, addbuttons=addbuttons
            )
        except:
            if self.channel != None:
    ...
    async def play_song(self, ctxmode: Union[discord.TextChannel, commands.Context]):
        ...
        voice, ctx = (
            self.voice,
            ctxmode,
        )  # discord.utils.get(self.bot.voice_clients, guild=interaction.guild)
        if self.songs:
            ...
            if self.current.state != "Ok":
                ...
                if self.current.state == "Error":
                    ...
                    self.internal_message_log.append(
                        f"I could not play {self.current.title} : `{str(self.current.error_value)}`"
                    )
            ...
            if song.state == "Error":
                ...
                self.internal_message_log.append(
                    f"I could not play {song.title} : `{str(song.error_value)}`"
                )
                asyncio.run_coroutine_threadsafe(
                    self.player_actions("auto_next"), self.bot.loop
                )
            ...
            voice.is_playing()
            self.bot.add_act(
                "MusicPlay", f"{song.title}", discord.ActivityType.listening
            )
            await self.send_message(ctx, "play", f"**{song.title}** is now playing.  ")

    async def play_song_override(
        self, ctxmode: Union[discord.TextChannel, commands.Context], override
    ):
    ...
    def get_music_embed(self, title: str, description: str) -> discord.Embed:
        ...
        embed = discord.Embed(
            title="", description=self.old_desc, color=Colour(0x68FF72)
        )
        myname = AssetLookup.get_asset("name")
        myicon = AssetLookup.get_asset("embed_icon")
        embed.set_author(name=f"{myname}'s music player.", icon_url=myicon)
        ...
        con = ""
        plistc = ""
        if self.player_condition == "play":
            con = "Playing Song."
        elif self.player_condition == "pause":
            con = "Player is Paused."
        elif self.player_condition == "stop":
            con = "Player Stopped."
        if self.repeat:
            plistc = " Repeat is on."
        elif self.repeatone:
        ...
        if self.current != None:
            ...
            title, url, duration = (
                song.title,
                song.url,
                seconds_to_time_stamp(song.duration),
            )
