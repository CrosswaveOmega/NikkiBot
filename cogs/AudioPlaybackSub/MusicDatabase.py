...
from sqlalchemy import Column, ForeignKey, Integer, String, Double, Text, JSON, and_, or_, cast, Float
...
class MusicJSONMemoryDB(MusicBase):
    ...
    def count_total_substring_matches(cls,parts:List[str],session=None) -> List[Tuple[str, int]]:
        ...
        part_conditions = [
            ...
            for part in parts if part and not part.isspace()
        ]
        ...
        part_matches = [(part, count) for part, count in zip(parts, part_counts_query) if count>0]
    ...
    def max_search(cls, sub_count:List[Tuple[str,int]],session=None)->List:
        ...
        sum_of_counts = sum(func.coalesce(case_statement, 0) for case_statement in case_statements)

        # Query to get the final results and sort by the sum
        query = session.query(
            cls, sum_of_counts.label("total_count")
            ).group_by(cls).order_by(sum_of_counts.desc()).limit(10)

        # Execute the query and fetch all results
        results = query.all()

        maxdistinct_matches = [(result[0], result[1]) for result in results if  result[1]>0]
    ...
    def max_search_tup(cls, parts):
        ...
        query = session.query(cls.url, *case_statements).group_by(cls.url).order_by(func.count().desc()).limit(10)
    ...
    def search(cls, query:str,do_sub:bool=True, do_maxsearch:bool=False):
        ...
        if do_sub:
            ...
            if is_url(query):
                ...
                if url_components.scheme and url_components.netloc:  # Ensure it's a valid URL
