...
def create_rrule(dtvals):
    ...
    try:
        rrule_obj = rrule(freq, byweekday=weekday_constants, interval=interval, dtstart=times)
        return rrule_obj
...
def create_rrule_embed(dtvals):
    ...
    if isinstance(rrule_obj,str):
        return discord.Embed(title="Error", description=rrule_obj, color=discord.Color.red())
    ...
    embed = discord.Embed(title="Current RRule Object", 
                          description=expl+"\n"+f"{sent}\n`{str(rrule_obj)}`",
                          color=discord.Color.green())
    embed.add_field(name="Next four estimated runtimes", value="\n".join(map(str, timestamps)), inline=False)
...
class RRuleView(discord.ui.View):
    ...
    @discord.ui.select(placeholder="Select Frequency", options=[
        discord.SelectOption(label="Daily",description="This task will run every day.", value="daily")
        ,discord.SelectOption(label="Weekly",description="This task will run on a weekly basis.", value="weekly")
    ...
    @discord.ui.button(label=f'Change Time ({timedef})', style=discord.ButtonStyle.blurple, row=3)
    async def timeedit(self, interaction: discord.Interaction, button: discord.ui.Button):
        '''
        ...
        if name_modal.done!=None:
            ...
            try:
                ...
                await interaction.edit_original_response(
                    content="time is valid", embed=self.emb()
                    ,view=self
                    )
            ...
            await interaction.edit_original_response(content='Time changing has been cancelled.', embed=self.emb())

    @discord.ui.button(label='Change Interval \n(1)', style=discord.ButtonStyle.blurple, row=3)
        ...
        if name_modal.done!=None:
            ...
            try:
                ...
                if ival<=0:
                    ...
                    raise ValueError(f"For the sake of my sanity, please keep this number below 10.")
            ...
            await interaction.edit_original_response(content='Interval Edit Cancelled.', embed=self.emb())
    ...
    async def confirm(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        if isinstance(rrule_obj,str):
            await interaction.response.edit_message(content=f'There is an issue with your input!\n{rrule_obj}',embed=self.emb())
        else:
            await interaction.response.edit_message(content="complete ",embed=self.emb())
