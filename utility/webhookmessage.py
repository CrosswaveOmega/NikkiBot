...
class WebhookMessageWrapper:
    ...
    async def postCopyWithWebhook(webhook,thread,  message:discord.Message, embeds=List[discord.Embed],noauthor=False):
        ...
        return await WebhookMessageWrapper.postMessageWithWebhook(webhook,thread, message_content=content, display_username=name, avatar_url=avatar, embed=embeds, file=files)
    ...
    async def getWebhookInChannel(text_channel:discord.TextChannel):
        ...
        if(webhook==None):
            ...
            webhook=await text_channel.create_webhook(name=newname, avatar=pfp, reason="So proxy messages can be sent in this channel.")
            fp.close()
    ...
    async def postMessageWithWebhook(webhook,thread,message_content, display_username, avatar_url, embed=[],file=[]):
        ...
        try:
            if thread!=None:
                mess=await webhook.send(content=newContent, username=display_username,avatar_url=avatar_url, embeds=embed, files=file, wait=True, thread=thread) 
                return mess
            else:
                mess=await webhook.send(content=newContent, username=display_username,avatar_url=avatar_url, embeds=embed, files=file, wait=True)
                return mess
        except:
            if thread!=None:
                mess=await webhook.send(content=newContent, username="CaptainHook",avatar_url=avatar_url, embeds=embed, files=file, wait=True, thread=thread)
                return mess
            else:
