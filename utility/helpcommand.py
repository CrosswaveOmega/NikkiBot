        ...
        for i, v in self.cogs.items():
            options.append(
                discord.SelectOption(
                    ...
                    description="This is cog {}".format(v.qualified_name),
                )
            )
        ...
        for i, v in self.cogs.items():
            if value == v.qualified_name:
                ...
                await interaction.response.edit_message(
                    content="showing cog help", embed=myembed, view=self.view
                )
...
class Chelp(HelpCommand):
    ...
    async def get_cog_help_embed(self, cog):
        ...
        channel = ctx.channel
        embed = discord.Embed(
            title="Help", colour=discord.Colour(0x7289DA), description="Cog_Help"
        )
        gui.gprint(cog)
        commandSTR = ""
