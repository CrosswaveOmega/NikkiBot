...
class AuditProfile(AIBase):
    ...
    def get_or_new(server,user):
        ...
        if sa==None: sa=AuditProfile.add(server.id,'server')
        if ua==None: ua=AuditProfile.add(user.id,'user')
        return sa,ua
...
class ServerAIConfig(AIBase):
    ...
    def add_channel(self, channel_id):
        session = DatabaseSingleton.get_session()
        channel = session.query(EnabledChannel).filter_by(server_id=self.server_id, channel_id=channel_id).first()
        if not channel:
    ...
    def list_channels(self):
        session = DatabaseSingleton.get_session()
        enabled_channels = session.query(EnabledChannel).filter_by(server_id=self.server_id).all()
        return [channel.channel_id for channel in enabled_channels]
    
    def count_channels(self):
        session = DatabaseSingleton.get_session()
        count = session.query(EnabledChannel).filter_by(server_id=self.server_id).count()
        return count
    
    
    def add_message_to_chain(self, message_id, created_at, role=None, content=None, name=None, function=None):
        session = DatabaseSingleton.get_session()
        message_chain = MessageChain(server_id=self.server_id, message_id=message_id, created_at=created_at, role=role, content=content, name=name, function=function)
        session.add(message_chain)
        session.commit()

    def remove_message_chain(self, message_id):
        session = DatabaseSingleton.get_session()
        message_chain = session.query(MessageChain).filter_by(server_id=self.server_id, message_id=message_id).first()
        if message_chain:
