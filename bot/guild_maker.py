...
async def new_guild(bot):
    ...
    guild_icon_url = bot.application.icon.url # Replace with the URL of the desired guild icon
    ...
    channels = ["updates", "resources", "internal_debug", "error_log", "general", "off-topic", "voicechannel"]
    ...
    await user.send(f"Hello world!  I've created a new guild \"{guild.name}\" with an invite: {invite.url}")
