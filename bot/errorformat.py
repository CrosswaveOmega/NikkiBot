...
def client_error_message(error_main, name="command"):
    ...
    if isinstance(error, discord.ext.commands.errors.CommandNotFound):
        return f"`{name}` is not a valid command, sorry.", True
    if isinstance(error, discord.app_commands.BotMissingPermissions) or isinstance(
        ...
        if len(error.missing_permissions) > 1:
            missing += "s"
        return f"I'm sorry, but I can not invoke {name} without the {missing}.", False
    if isinstance(error, discord.ext.commands.errors.CheckFailure):
        return f"{name} failed a check.", False
    else:
        return f"{name} failed due to {str(error)}", False
